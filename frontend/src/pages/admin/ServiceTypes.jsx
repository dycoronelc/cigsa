  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalMode === 'edit' && editingId) {
        await api.put(`/service-types/${editingId}`, {
          name: formData.name,
          isActive: formData.isActive
        });
      } else {
        await api.post('/service-types', { name: formData.name });
      }
      closeModal();
      fetchItems();
    } catch (error) {
      showError(error.response?.data?.error || 'Error al guardar tipo de servicio');
    }
  };

  const handleDelete = async (id) => {
    showConfirm('¿Está seguro de desactivar este tipo de servicio?', async () => {
      try {
    ...
    <div className="management-page">
      ...
      <div className="table-header">
        ...
        <div className="table-header-actions">
          <button onClick={openCreateModal} className="btn-primary">+ Nuevo Tipo</button>
        </div>
      </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h2>{modalMode === 'edit' ? 'Editar Tipo de Servicio' : 'Nuevo Tipo de Servicio'}</h2>
